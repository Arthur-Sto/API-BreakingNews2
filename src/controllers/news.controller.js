...
export const findAll = async (req, res) => {
    try {
        ...
        if (news.lenght === 0) {
            return res.status(400).send({ message: "There are no registared news" })
        }
        res.send({
            nextUrl,
            previousUrl,
...
export const update = async (req, res) => {
    try {
        ...
        if (String(news.user._id) !== String(req.userId)) {
            return res.status(403).send({ message: "You don't have permission to update this post" });
        }
...
export const erase = async (req, res) => {
    try {
        ...
        const news = await findByIdService(id);
        if (String(news.user._id) !== String(req.userId)) {
            return res.status(403).send({ message: "You don't have permission to delete this News" });
        }
